...
def test_add_event_by_command(ctrl: CalendarController) -> None:
    ctrl.add_event("test", "1.11:2023", "11-22")
    event = ("test", str(dt.datetime(2023, 11, 1, 11, 22)), False)
    assert ctrl.model._events_data == [event]


def test_read_event_from_json(model: CalendarModel, datetime: dt.datetime) -> None:
    data = {"events": [["test", str(datetime), False]]}
    with open(TEST_JSON_PATH, "w", encoding="utf-8") as f:
        json.dump(data, f)
    model._load_settings()

    assert model.calendar_data == [Event("test", datetime, False)]
...
def test_event_full_name(monkeypatch: MonkeyPatch) -> None:
    event = Event("test1", dt.datetime(2023, 1, 23, 9, 34), False)
    assert event.full_name == "**test1** (9:34)"
    event = Event("test2", dt.datetime(2023, 1, 23, 0, 0), True)
    assert event.full_name == "**test2**"
    event = Event("test3", dt.datetime(2023, 1, 23, 0, 0), False)
    assert event.full_name == "**test3** (0:00)"
...
def test_events_with_indexes(
    ...
    event1 = model.add_event_to_json("test1", datetime, True)
    event2 = model.add_event_to_json("test2", datetime, True)
